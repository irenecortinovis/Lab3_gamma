...
//c++  easyfit_peak1_teo.cpp  `root-config --cflags --glibs` -o easyfit_peak1_teo

void easyfit_peak1(char *nomefile)
{
	//gROOT->Reset();
...
// 	fit e plot dell'istogramma
  ...
  double maximum = fitfunc->GetMaximum(5885,5950);
  std::cout << "maximum value \t" << maximum << std::endl;

  TF1* diffunc = new TF1 ("diffunc","abs(-[8] + fitfunc)",5885,5950);	//Calcolo la FWHM su fitfunc senza sottrarre il "fondo" all'istogramma
  diffunc -> SetNpx (100000);
  diffunc -> FixParameter(8,maximum*0.5);
