...
int main(int argc, char **argv) {
  ...
  TGraphErrors *graph_depths3 = new TGraphErrors(x_depth.size(), &x_depth[0], &y_peak3[0]);
	...
	graph_depths1->SetTitle("Attenuation for copper peak1; #depth (#mm); N_{counts}");
  ...
  TF1 *expon2 = new TF1("fd2", "[0]*(exp(-[1]*x))", 0., 110.);
	...
	graph_depths2->SetTitle("Attenuation for copper peak2; #depth (#mm); N_{counts}");
  ...
  TF1 *expon3 = new TF1("fd3", "[0]*(exp(-[1]*x))", 0., 110.);
	...
	graph_depths3->SetTitle("Attenuation for copper peak K40; #depth (#mm); N_{counts}");
